...
// NewExchangeController 交易所控制器构造函数
func NewExchangeController(marketDataService *services.MarketDataService) *ExchangeController {
...
func (e *ExchangeController) GetPrice(ctx context.Context, c *app.RequestContext) {
	...
	if e.marketDataService != nil {
		e.marketDataService.SavePrice(string(exchange), pair, price)
	}
...
func (e *ExchangeController) GetAllPrices(ctx context.Context, c *app.RequestContext) {
	...
	if e.marketDataService != nil {
		for _, p := range prices {
			if p.Error == "" && p.Price != "" {
				e.marketDataService.SavePrice(string(p.Exchange), p.Pair, p.Price)
			}
		}
	}
